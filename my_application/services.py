...
async def get_tweet_one(session: {execute}, id_tweets: int, api_key: Optional[str]):
    tweet = await session.execute(select(
...
async def info_user_id(session: {execute}, id_user: int):
    user = await session.execute(select(User).options(selectinload(User.following)).where(
...
async def get_user_following(session: {execute}, user: Any):
    following = await session.execute(select(User).join(
        user_following, user_following.c.followers == User.id).where(
            user_following.c.following == cast(user.id, Integer)))
    following_list = following.scalars().all()
    following = [i.to_json() for i in following_list]
    result = user.get_user_json()
...
async def get_likes(session: {execute}, user: Any, tweet: Any):
    like = await session.execute(select(Like).where(
...
async def upload_media(file: UploadFile, request) -> Optional[str]:
    content = file.filename.replace(" ", "")
    url = f'{request.url}/image/{content}'
    path = os.path.join(base_dir, 'my_application', 'storage', 'media', content)
    file.file.seek(0, 2)
